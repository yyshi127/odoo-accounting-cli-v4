"""Narrow bridge port for fixed sales and purchase order reads."""
...
class OdooOrderDocumentsPort:
    """Invoke only the fixed order-document read action."""
    ...
    def user_id(self) -> int:
        if self._user_id is None:
            raise ValueError("No verified Odoo order-document page has been read.")
    ...
    def read(
        ...
        *,
        ...
        if capability_id not in CAPABILITY_IDS:
            raise ValueError("Unsupported order-document capability.")
        self._user_id = None
        ...
        if not _valid_page(page):
            raise ValueError("The Odoo bridge returned an invalid order-document page.")

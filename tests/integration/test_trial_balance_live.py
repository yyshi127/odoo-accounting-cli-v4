"""Live read-only verification for the fixed trial-balance report."""
...
def _invoke(alias: str, company_id: int) -> dict:
    ...
    request_id = str(
        uuid.uuid5(uuid.NAMESPACE_URL, f"odacv4:{alias}:{company_id}:trial-balance")
    )
    request = {
        "schema_version": "v1",
    ...
    registry.validate_instance(
        "schemas/v1/report.trial_balance.request.schema.json", request
    )
...
def test_trial_balance_uses_the_real_odoo_readonly_report_handler(
    ...
    data = document["data"]
    assert data["report"]["key"] == "trial_balance"
    ...
    assert len({line["id"] for line in data["lines"]}) == 3
    ...
    assert len(totals) == 1
    assert totals[0]["values"] == ["0", "123.45", "123.45", "0"]

"""Live read-only verification for the fixed profit-and-loss report."""
...
def _invoke(alias: str, company_id: int) -> dict:
    ...
    request_id = str(
        uuid.uuid5(uuid.NAMESPACE_URL, f"odacv4:{alias}:{company_id}:profit-and-loss")
    )
    request = {
        "schema_version": "v1",
    ...
    registry.validate_instance(
        "schemas/v1/report.profit_and_loss.request.schema.json", request
    )
    ...
    registry.validate_instance(
        "schemas/v1/report.profit_and_loss.response.schema.json", document
    )
...
def test_profit_and_loss_uses_the_real_odoo_readonly_report(
    ...
    assert values == {
        "Revenue": ["0"],
        "Less Costs of Revenue": ["0"],
        "Gross Profit": ["0"],
        "Less Operating Expenses": ["123.45"],
        "Operating Income (or Loss)": ["-123.45"],
        "Plus Other Income": ["0"],

"""Live read-only verification for the fixed cash-flow report."""
...
def _invoke(alias: str, company_id: int) -> dict:
    ...
    request_id = str(
        uuid.uuid5(uuid.NAMESPACE_URL, f"odacv4:{alias}:{company_id}:cash-flow")
    )
    request = {
        "schema_version": "v1",
    ...
    registry.validate_instance("schemas/v1/report.cash_flow.request.schema.json", request)
...
def test_cash_flow_uses_the_real_odoo_readonly_report(
    ...
    data = document["data"]
    assert data["report"] == {"key": "cash_flow", "name": "Cash Flow Statement"}
    ...
    assert len({line["id"] for line in data["lines"]}) == 19
    ...
    assert values["Cash and cash equivalents, beginning of period"] == ["0"]
    assert values["Net increase in cash and cash equivalents"] == ["-123.45"]
    assert values["Cash flows from unclassified activities"] == ["-123.45"]
    assert values["Cash and cash equivalents, closing balance"] == ["-123.45"]
    bank_lines = [
        line
        for line in data["lines"]
        if line["parent_id"] is not None and line["name"].endswith(" Bank")
    ]
    assert len(bank_lines) == 1
    assert bank_lines[0]["unfoldable"] is False
